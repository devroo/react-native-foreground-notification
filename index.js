...
// ANDROID ONLY
// Copied and adapted from https://github.com/devroo/react-native-foreground-notification
...
class ForegroundService {
	...
	static async stopServiceAll() {
		console.log('StopAll Service Triggered')
	...
	static async runTask(taskConfig) {
		console.log('runTask Service Triggered')
	...
	static async isRunning() {
		console.log('isRunning Service Triggered')
	...
	try {
		...
		Object.entries(tasks).forEach(([taskId, task]) => {
			...
			if (now >= task.nextExecutionTime) {
				...
				promises.push(Promise.resolve(task.task()).then(task.onSuccess, task.onError))
...
const register = () => {
	if (!serviceRunning) return ForegroundService.registerForegroundTask('myTaskName', taskRunner)
}
...
const add_task = (task, { delay = 5000, onLoop = true, taskId = randHashString(12), onSuccess = () => {}, onError = () => {} }) => {
	const _type = typeof task
	if (_type !== 'function') throw `invalid task of type ${_type}, expected a function or a Promise`
...
const update_task = (task, { delay = 5000, onLoop = true, taskId = randHashString(12), onSuccess = () => {}, onError = () => {} }) => {
	const _type = typeof task
	if (_type !== 'function') throw `invalid task of type ${_type}, expected a function or a Promise`
